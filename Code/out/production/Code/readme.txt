...
After validation, the makeNumber method is called to build a phone number string in the E164 format. I am using a string builder along with character arrays for more efficiency as creating a new string for each digit addition is not good practice. At this time the characters, except the digits, are ignored which are then appended to the string builder. Finally, a check is performed on the lenght of the string to bound it to 12 characters after addition of '+1' for the US CC.
